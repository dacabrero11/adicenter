...
export function Sistemas() {
  ...
  const toggleFicha = (n: string) => setFichaAbierta((v) => (v === n ? null : n));
  return (
      ...
      <section id="sistemas" className="relative z-2 py-19 lg:py-28">
        <div className="mx-auto max-w-[1220px] px-6">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {LINEAS.map((l, i) => (
              <div key={l.n} className="flex flex-col overflow-hidden rounded-[6px] border border-navy/[.13] transition-colors">
              <Reveal
                index={i}
                as="article"
                className="card-live card-live-light border-breathe group flex flex-col bg-white transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_30px_60px_-32px_rgba(1,35,135,.4)]"
              >
                ...
                <div className="flex flex-1 flex-col px-7 pb-7 pt-6">
                    ...
                    {l.specs.map(([icon, k, v]) => (
                      ...
                      >
                        <span className="flex min-w-0 items-center gap-2 text-ink-soft">
                          ...
                          <span className="uppercase tracking-[0.02em] truncate">{k}</span>
                        </span>
                  ...
                  <button
                    onClick={() => toggleFicha(l.n)}
                    aria-expanded={fichaAbierta === l.n}
                  ...
                  >
                    {fichaAbierta === l.n ? "Cerrar ficha" : "Ver aplicaciones y especificaciones"}
                    <span className={`inline-block transition-transform duration-300 ${fichaAbierta === l.n ? "rotate-90" : ""}`}>{I.arrow}</span>
                  </button>
                </div>
              </Reveal>
              {fichaAbierta === l.n && (
                <div className="border-t bg-white px-7 py-4" style={{ borderColor: `${l.accent}44` }}>
                  <p className="font-mono-adi mb-3 text-[11px] uppercase tracking-[0.14em]" style={{ color: l.accent }}>
                    ...
                    <div className="mb-3 grid grid-cols-2 gap-x-6 gap-y-2 sm:grid-cols-3">
                      {l.specs.map(([, k, v]) => (
                        <div key={String(k)} className="min-w-0">
                          <div className="font-mono-adi text-[11px] uppercase tracking-[0.06em] text-ink-soft">{String(k)}</div>
                          <div className="mt-0.5 text-[13px] font-semibold text-ink">{String(v)}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    <a
                      href="#catalogo"
                      className="font-mono-adi inline-flex items-center gap-1.5 rounded-[4px] border px-3.5 py-1.75 text-[11px] uppercase tracking-[0.08em] transition-colors hover:opacity-80"
                      style={{ color: l.accent, borderColor: `${l.accent}66` }}
                    >
                      Ver todos los servicios {I.arrow}
                    </a>
                    <a
                      href="#cotizar"
          ...
          >
            ...
            <a
              href="#proyectos"
              onClick={(e) => { e.preventDefault(); (window as any).__scrollToHash?.("#proyectos"); const el = document.getElementById("proyectos"); if(el) el.scrollIntoView({behavior:"smooth"}); }}
