...
/* -------- Componente aislado para la animación de J1MMY -------- */
function J1MMYAnimado() {
    ...
    >
      <video
        ...
        style={{
          ...
          filter: "drop-shadow(0 24px 38px rgba(1,35,135,.28))",
        }}
      />
    </div>
  );
...
const OPCIONES_RAPIDAS = [
  ...
  { label: "Otro problema", icon: I.search, texto: "Tengo otro problema, quiero contarle a J1MMY" },
];

export function JimmySection({ onAskJ1MMY }: { onAskJ1MMY: (mensaje?: string) => void }) {
  return (
      ...
      <div className="relative z-2 mx-auto max-w-[1260px] px-6 py-19 lg:py-24">
        ...
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[.85fr_1.15fr] lg:gap-8">
          <Reveal as="div" className="flex items-center justify-center gap-2 sm:gap-5">
            {/* indicadores técnicos — solo desde lg, junto a J1MMY */}
            <div className="hidden flex-col border-r border-navy/[.14] pr-5 lg:flex">
              <h3 className="font-display mb-6 text-[15px] font-bold uppercase tracking-[0.06em] text-navy">
                Etapas del proceso
              </h3>
            ...
            <div className="relative flex-none">
              ...
              />
              <J1MMYAnimado />
              <div className="absolute right-0 top-0 z-10 -translate-y-full -translate-x-4 mb-3 flex items-center gap-1.5 rounded-full border border-navy/[.14] bg-white px-2.5 py-1.25 shadow-[0_8px_20px_-8px_rgba(1,35,135,.35)]">
          ...
          <Reveal as="div" index={1}>
            ...
            <p className="mt-5 max-w-[54ch] text-base text-ink-soft sm:text-[18px]">
              J1MMY responde preguntas técnicas, orienta sobre sistemas, calcula consumos y sabe
              cuándo una consulta necesita pasar de la pantalla a una visita en obra.
            </p>
          </Reveal>
        </div>

        {/* ---- J1MMY en obra: refuerza el "anda en la obra" del encabezado ---- */}
        <div className="mt-11 grid grid-cols-1 gap-4.5 sm:grid-cols-3">
          {[
            ["cubierta", "Impermeabilización de cubierta", "Aplicación de membrana sobre losa preparada."],
            ["sellado", "Sellado de puntos críticos", "Encuentros y bases donde se pierde el trabajo."],
            ["membrana", "Membrana asfáltica", "Instalación en rollo con soplete."],
          ].map(([img, titulo, texto], i) => (
            <Reveal
              key={img}
              as="article"
              index={i + 2}
              className="group relative overflow-hidden rounded-[8px] border border-navy/[.13] shadow-[0_20px_40px_-32px_rgba(1,35,135,.4)] transition-all duration-300 hover:-translate-y-1 hover:border-cyan/45"
            >
              <div className="relative aspect-[4/3] w-full overflow-hidden sm:aspect-[3/2.6]">
                <Image
                  src={`/images/jimmy/obra/${img}.webp`}
                  alt={titulo}
                  fill
                ...
                <span
                  ...
                  style={{
                    background:
                      "linear-gradient(180deg,rgba(3,11,34,0) 42%,rgba(3,11,34,.88) 100%)",
                  }}
                />
              </div>
              <figcaption className="absolute inset-x-0 bottom-0 p-4">
                <span className="font-mono-adi block text-[12px] uppercase tracking-[0.08em] text-white">
                  {titulo}
                </span>
                <span className="mt-1 block text-[12px] leading-snug text-white/72">{texto}</span>
              </figcaption>
            </Reveal>
          ))}
        </div>
        ...
        <div className="mt-11 grid grid-cols-1 gap-4.5 sm:grid-cols-3">
          ...
          >
            ...
            <button
              onClick={() => onAskJ1MMY("Quisiera agendar una visita técnica en obra")}
        ...
        <Reveal as="div" index={5} className="mt-6 grid grid-cols-1 gap-4.5 lg:grid-cols-[1.45fr_1fr]">
          <div className="rounded-[8px] border border-navy/[.13] bg-white p-5.5 shadow-[0_20px_40px_-32px_rgba(1,35,135,.4)]">
            <div className="flex items-center gap-2.5">
              ...
              <strong className="font-display flex items-center gap-1.25 text-[14px] text-ink">
                J1MMY <span className="text-cyan">{I.check}</span>
              </strong>
            </div>
          ...
          >
            <div>
              ...
              <span className="mt-1.5 block text-[13px] text-sky">Hablar con J1MMY</span>
            </div>
