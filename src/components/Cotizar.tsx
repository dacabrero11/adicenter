        ...
        <div className="grid grid-cols-1 items-start gap-14 lg:grid-cols-[.85fr_1.15fr]">
            ...
            {estado === "ok" ? (
              <div className="rounded-[4px] border border-cyan/40 bg-cyan/8 p-10 text-center">
                ...
                <p className="mt-3 text-[15px] text-white/70">
                  Un asesor técnico le responde con el estimado formal en menos de 2 horas hábiles.
                </p>
                <button
                  type="button"
                  onClick={() => setEstado("idle")}
                  className="font-mono-adi mt-7 text-[11px] uppercase tracking-[0.13em] text-cyan underline-offset-4 hover:underline"
                >
          ...
          >
            ...
            <div className="mb-5.5">
              <label htmlFor="mensaje" className="font-mono-adi mb-2 block text-[10px] uppercase tracking-[0.15em] text-white/50">
...
function Field({ label, name, ...props }: { label: string; name: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div>
      <label htmlFor={name} className="font-mono-adi mb-2 block text-[10px] uppercase tracking-[0.15em] text-white/50">{label}</label>
      <input
        id={name}
        name={name}
        {...props}
